...
// //       assignedTo: task.assignedTo,
// //       errors: {},
// //       fileUrls: task.fileUrls.filter((url) => typeof url === "string" && url.startsWith("http")),
// //     });
// //     setEditId(task._id);
// //     setIsModalOpen(true);
// //   };
...
// //                               <span key={idx} title={emp.name} className="mr-1">
...
// //                               (a, b) => new Date(b.timestamp) - new Date(a.timestamp)
// //                             )
// //                             .slice(0, showFullComments ? viewTask.comments.length : 1)
// //                             .map((comment, index) => (
// //                               <div
// //                                 key={index}
// //                                 className={`mb-3 pb-2 border-b border-gray-200 last:border-b-0 last:mb-0 ${
...
// //                         {viewTask.activityLogs && viewTask.activityLogs.length > 0 ? (
// //                           viewTask.activityLogs
// //                             .slice(0, showFullComments ? viewTask.activityLogs.length : 1)
// //                             .map((log, index) => (
// //                               <div key={index} className="mb-2 last:mb-0">
// //                                 <p className="text-gray-700">
// //                                   {log.message || "No message"}
...
// // Function to generate a consistent color based on name for the circle
...
//                             className={`inline-flex items-center justify-center w-6 h-6 rounded-full ${getColorForName(
//                               task.assignedBy
        ...
        <main className="flex-1 p-4 sm:p-6 overflow-auto">
          <div className="max-w-8xl mx-auto">
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6  ">
                  ...
                  <div className="max-h-[500px] overflow-y-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-white sticky top-0 z-10 border-b border-gray-200">
                        <tr>
                          <th className="text-left py-6 px-6 sm:px-8 font-medium text-gray-700">
                            Task ID
                          </th>
                          <th className="text-left py-6 px-6 sm:px-8 font-medium text-gray-700">
                            Assigned Date
                      ...
                      <tbody>
                        {sortedTasks.map((task) => (
                          ...
                          >
                            <td className="py-4 px-6 sm:px-8 text-gray-900">
                              {task.taskId}
                            </td>
                            <td className="py-4 px-6 sm:px-8 text-gray-600">
                              {task.assignedDateTime
                                ? new Date(
                                    task.assignedDateTime
                                  ).toLocaleDateString()
              ...
              {isViewModalOpen && viewTask && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                  <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl sm:max-w-2xl md:max-w-3xl lg:max-w-6xl p-4 sm:p-6 max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center space-x-2">
                        <h2 className="text-lg sm:text-xl font-semibold">
                          {viewTask.taskName}
